...
public class PlaylistManager {
    ...
    public String generateMultiVariantPlaylist(Media media) {
        ...
        for (QualityProfiles.QualityProfile profile : QualityProfiles.AllProfiles) {
            multiVariantPlaylist.append("#EXT-X-STREAM-INF:BANDWIDTH=").append(profile.getBandwidthUpperBound()).append(",RESOLUTION=").append(profile).append("\n");
            multiVariantPlaylist.append("/api/playlist/").append(media.getId()).append("/").append(profile.getName()).append("\n");
        }
    ...
    public String generateVodPlaylist(Media media, QualityProfiles.QualityProfile qualityProfile) {
        ...
        for (int i = 0; i < segments.size(); i++) {
            ...
            String uri = String.format("/api/video/%s/%s/segment%05d.ts", media.getId(), qualityProfile.getName(), i);
    ...
    public List<Double> getSegments(KeyframeData keyframeData) {
        ...
        for (double keyframe : keyframes) {
            if (keyframe < desiredCutTime) continue;
